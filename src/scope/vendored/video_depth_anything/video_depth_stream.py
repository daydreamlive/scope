...
from .vda_utils.util import compute_scale_and_shift, get_interpolate_frames
...
class VideoDepthAnything(nn.Module):
    ...
    def infer_video_depth_one(
        ...
        # Note: use_temporal_cache is accepted for API compatibility but VDA
        # streaming always uses temporal caching by design. Set to False to
        # force single-frame mode (resets cache each frame - slower but stateless).
        if not use_temporal_cache:
            # Stateless mode: reset cache to force first-frame path each time
            self.transform = None
            self.frame_cache_list = [None]
        ...
        if self.transform is None:  # first frame
            ...
            new_depth_t = depth[0][0]
            new_depth = new_depth_t if return_torch else new_depth_t.cpu().numpy()
        else:
            frame_height, frame_width = frame.shape[:2]
