"""
Unified test script for Krea Realtime Video pipeline with VACE integration.
...
Usage:
    ...
    python -m scope.core.pipelines.krea_realtime_video.test_vace
"""
...
CONFIG = {
    ...
    # ===== QUANTIZATION =====
    # Set to Quantization.FP8_E4M3FN to enable FP8 quantization for 5090 support (32GB VRAM)
    # Set to None to disable quantization (requires more VRAM)
    "quantization": Quantization.FP8_E4M3FN,
    # ===== INPUT PATHS =====
    # R2V: List of reference image paths
    "ref_images": [
        "C:/Users/ryanf/.daydream-scope/images/example.png",  # path/to/image.png
    ],
    ...
    "num_chunks": 4,  # Number of generation chunks
    "frames_per_chunk": 12,  # Frames per chunk (3 latent * 4 temporal upsample)
    "height": 128,  # Krea default height
    "width": 128,  # Krea default width
    "vace_context_scale": 1.0,  # VACE conditioning strength (0.0-1.0)
    # ===== INPAINTING SPECIFIC =====
    "mask_threshold": 0.5,  # Threshold for binarizing mask (0-1)
    "mask_value": 127,  # Gray value for masked regions (0-255)
    # ===== OUTPUT =====
    "output_dir": "vace_tests/krea",  # path/to/output_dir
}
...
def main():
    print("=" * 80)
    print("  Krea Realtime Video Unified Test Script")
    print("=" * 80)
    ...
    quantization_str = (
        str(config["quantization"]).split(".")[-1] if config["quantization"] else "None"
    )
    print(f"  Quantization: {quantization_str}")
    ...
    vace_path = str(
        get_model_file_path("Wan2.1-VACE-14B/diffusion_pytorch_model.safetensors")
    )
    ...
    for chunk_index in range(config["num_chunks"]):
        ...
        if use_depth:
            ...
            kwargs["input_frames"] = depth_chunk
