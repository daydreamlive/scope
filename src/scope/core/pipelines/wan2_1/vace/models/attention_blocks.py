        ...
        def forward(
            ...
            **kwargs,
            ...
            if hints is not None and self.block_id is not None:
                ...
                hint_checksum_before_scale = hint.abs().sum().item()
                x_before_hint = x.abs().sum().item()
                x = x + hint * context_scale
                x_after_hint = x.abs().sum().item()
                print(
                    f"BaseWanAttentionBlock.forward: INJECTING HINT: block_id={self.block_id}, "
                    f"hint_index={self.block_id}, hint_checksum={hint_checksum_before_scale:.6f}, "
                    f"x_before={x_before_hint:.6f}, x_after={x_after_hint:.6f}, scale={context_scale}"
                )
