...
class VaceSelfAttention(nn.Module):
    ...
    This replaces flex_attention + padding-to-128 with a direct flash_attention
    call, eliminating:
    - Padding to multiple of 128 (3 tensor allocations + 3 cat ops per call)
    - flex_attention compiled-graph overhead (and its Inductor max-autotune
      NoValidChoicesError on certain padded seq_lens with Wan's 12-head layout)
    - block_mask processing
    - .item() GPU-CPU sync for the is_tf check
...
def swap_vace_self_attention(vace_blocks, rope_apply_fn=None, flash_attention_fn=None):
    ...
    Args:
        ...
        rope_apply_fn: The rope_apply function from the pipeline's model module.
            If None, auto-discovers from the self_attn module's globals via MRO walk.
    ...
    if rope_apply_fn is None or flash_attention_fn is None:
        ...
        if hasattr(first_block, "self_attn"):
            ...
            # Walk MRO to find a module that defines rope_apply / flash_attention.
    ...
    for block in vace_blocks:
        if hasattr(block, "self_attn"):
            ...
            swapped += 1
    logger.info(
        f"swap_vace_self_attention: replaced self-attention on {swapped}/{len(vace_blocks)} VACE blocks with flash_attention"
    )
