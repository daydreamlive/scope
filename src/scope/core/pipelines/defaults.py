...
def apply_mode_defaults_to_state(
    ...
    """
    kwargs = kwargs or {}
    mode_config = get_mode_defaults(pipeline_class, mode)

    # Apply denoising steps if not provided
    if "denoising_step_list" not in kwargs and mode_config.denoising_steps:
        state.set("denoising_step_list", mode_config.denoising_steps)
