...
def get_vace_path():
    """Check for VACE checkpoint and return path if available."""
...
def test_pipeline(
    ...
    Args:
        ...
        ref_images: Optional list of reference image paths for VACE conditioning (longlive only)
        vace_context_scale: VACE context scale factor (default: 1.0, longlive only)
    """
    print(f"\n{'='*80}")
    print(f"Testing {pipeline_name} pipeline")
    print(f"{'='*80}\n")

    # Initialize VACE path (only used for longlive)
    vace_path = None
    ...
    if pipeline_name == "longlive":
        ...
        # Check for VACE checkpoint
        vace_path = get_vace_path()
        if vace_path:
            print(f"compare_v2v: Using VACE model from {vace_path}")
        elif ref_images:
            print(
                "compare_v2v: Warning - VACE checkpoint not found but ref_images provided. "
                "VACE conditioning will be disabled."
            )
            print(
                "compare_v2v: VACE checkpoint expected at: "
                f"{Path.home() / '.daydream-scope' / 'models' / 'Wan2.1-VACE-1.3B' / 'diffusion_pytorch_model.safetensors'}"
            )
    ...
    for i in range(num_chunks):
        ...
        if pipeline_name == "krea":
            ...
            # Add VACE conditioning only on first chunk (VACE context is cached)
            if (
                pipeline_name == "longlive"
                and ref_images
                and vace_path is not None
                and i == 0
            ):
                kwargs["ref_images"] = ref_images
                kwargs["vace_context_scale"] = vace_context_scale
                print(
                    f"compare_v2v: Using VACE conditioning with {len(ref_images)} reference image(s), scale={vace_context_scale}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Reference image paths for VACE conditioning (longlive only). Can specify multiple images.",
    )
    parser.add_argument(
        "--vace-context-scale",
        type=float,
        default=1.0,
        help="VACE context scale factor (default: 1.0, longlive only). Controls strength of reference image conditioning.",
    )

    args = parser.parse_args()

    # If no ref_images provided but example.png exists, try to use it
    ...
    if ref_images is None:
        ...
        if example_img.exists():
            ...
            print(f"compare_v2v: Using example image: {example_img}")
