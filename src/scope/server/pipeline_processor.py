...
class PipelineProcessor:
    ...
    def __init__(
        ...
        self.input_frame_times = deque(maxlen=INPUT_FPS_SAMPLE_SIZE) if track_input_fps else None
        ...
        self.vace_enabled = getattr(self.pipeline, "vace_enabled", False)
        self.vace_use_input_video = (initial_parameters or {}).get("vace_use_input_video", True)
    ...
    def worker_loop(self):
        ...
        while self.running and not self.shutdown_event.is_set():
            try:
                ...
                logger.debug(f"Pipeline {self.pipeline_id} temporarily unavailable: {e}")
                ...
                if self._is_recoverable(e):
                    logger.error(
                        f"Error in worker loop for {self.pipeline_id}: {e}", exc_info=True
                    )
                    continue
                else:
                    logger.error(
    ...
    def process_chunk(self):
        ...
        try:
            ...
            if video_input is not None:
                if self.vace_enabled and self.vace_use_input_video:
