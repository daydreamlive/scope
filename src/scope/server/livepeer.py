...
class LivepeerConnection:
    ...
    def __init__(self):
        ...
        self._stats = {
            "connected_at": None,
            "frames_sent_to_livepeer": 0,
            "frames_received_from_livepeer": 0,
        }
    ...
    async def connect(
        ...
        token = os.environ.get(LIVEPEER_TOKEN_ENV)
        if not token:
            raise RuntimeError(
    ...
    def send_frame(self, frame: VideoFrame | np.ndarray) -> bool:
        ...
        if success:
            self._stats["frames_sent_to_livepeer"] += 1
        return success
    ...
    def _on_frame_from_livepeer(self, frame: VideoFrame) -> None:
        self._stats["frames_received_from_livepeer"] += 1
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        if self.webrtc_connected:
            status["stats"] = {
                ...
                "frames_sent_to_cloud": self._stats["frames_sent_to_livepeer"],
                "frames_received_from_cloud": self._stats[
                    "frames_received_from_livepeer"
                ],
            }
            if self._client is not None:
