...
class FrameProcessor:
    ...
    def notify_primary_frame_output(self, frame: torch.Tensor) -> None:
        ...
        output sinks (NDI/Spout). Called by the primary track's recv()
        when using get_from_sink() instead of get().
        """
        self._frames_out += 1
    ...
    def _setup_multi_input_sources(self, graph):
        ...
        Parses source nodes from the graph config for source_mode/source_name,
        and creates a separate InputSource + receiver thread for each
