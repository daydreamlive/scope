...
class FrameProcessor:
    ...
    def __init__(
        ...
        # Generic input source (plugin-based)
    ...
    def start(self):
        ...
        # Process generic input source settings (plugin-based)
    ...
    def update_parameters(self, parameters: dict[str, Any]):
        ...
        # Handle generic input source settings (plugin-based)
    ...
    def _update_input_source(self, config: dict):
        """Update generic input source configuration (plugin-based).

        Config format:
            {
                "enabled": bool,
                "source_type": str,  # e.g., "ndi"
                "source_name": str,  # e.g., "OBS (MacBook)"
            }
        """
        enabled = config.get("enabled", False)
        ...
        if enabled and not self.input_source_enabled:
            ...
            try:
                ...
                self.input_source = input_source_registry.create_instance(source_type)
                if self.input_source is None:
                    logger.error(f"Input source '{source_type}' not available")
                    return
                ...
                if self.input_source.connect(source_name):
                    ...
                    logger.error(
                        f"Failed to connect to input source: {source_type} -> '{source_name}'"
                    )
                    self.input_source.close()
                    self.input_source = None
            except Exception as e:
                logger.error(f"Error creating input source: {e}")
            ...
            # Source type changed or reconnect requested, recreate
            self.input_source_enabled = False
            if self.input_source is not None:
                self.input_source.close()
                self.input_source = None
            ...
            try:
                ...
                if self.input_source.connect(source_name):
                    ...
                    # Restart receiving thread if not running
                    if (
                        self.input_source_thread is None
                        or not self.input_source_thread.is_alive()
                    ):
                        self.input_source_thread = threading.Thread(
                            target=self._input_source_receiver_loop, daemon=True
                        )
                        self.input_source_thread.start()
                    logger.info(
                        f"Input source changed: {source_type} -> '{source_name}'"
                    )
                else:
                    logger.error(f"Failed to reconnect input source")
                    self.input_source.close()
                    self.input_source = None
            except Exception as e:
                logger.error(f"Error recreating input source: {e}")
                if self.input_source:
                    self.input_source.close()
                self.input_source = None
    ...
    def _input_source_receiver_loop(self):
        ...
        while (
            ...
            try:
                ...
                # Receive frame from input source
