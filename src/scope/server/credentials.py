# Copied from https://github.com/gradio-app/fastrtc/blob/main/backend/fastrtc/credentials.py
import os
...
def get_twilio_turn_credentials(twilio_sid=None, twilio_token=None):
    ...
    try:
        ...
        raise ImportError("Please install twilio with `pip install twilio`") from e
...
def get_turn_credentials(
    method: Literal["twilio", "cloudflare"] = "cloudflare", **kwargs
):
    ...
    Args:
        method (Literal["twilio", "cloudflare"], optional): The provider
            to use. 'cloudflare' uses either Cloudflare keys or HF token.
            'twilio' uses the Twilio API. Defaults to "cloudflare".
        **kwargs: Additional keyword arguments passed directly to the underlying
            provider-specific function (e.g., `twilio_sid`, `twilio_token` for 'twilio';
            `turn_key_id`, `turn_key_api_token`, `hf_token`, `ttl` for 'cloudflare').
    ...
    if method == "cloudflare":
        ...
        raise ValueError("Invalid method. Must be 'twilio' or 'cloudflare'")
...
async def get_turn_credentials_async(
    ...
    Args:
        method (Literal["twilio", "cloudflare"], optional): The provider
            to use. 'cloudflare' uses either Cloudflare keys or HF token. 'twilio' is not
            supported asynchronously by this function yet. Defaults to "cloudflare".
        **kwargs: Additional keyword arguments passed directly to the underlying
            provider-specific async function (e.g., `turn_key_id`, `turn_key_api_token`,
            `hf_token`, `ttl`, `client` for 'cloudflare').
