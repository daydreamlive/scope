...
def build_graph(
    ...
    # Backward compat: first sink determines the primary sink processor
    ...
    # Only for the backward-compat sink_processor when it isn't already
    # handled by per-sink queue setup above.
    already_resized = {
        e.from_node
    ...
    if sink_processor is not None and sink_node_id not in already_resized:
