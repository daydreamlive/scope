"""Reliable metrics reporter that forwards telemetry to Daydream /v1/metrics.
...
The class structurally satisfies the ``TelemetrySink`` protocol defined in
``kafka_publisher.py`` so it can be installed as the egress sink alongside
...
class MetricsReporter:
    ...
    Thread-safe: ``emit()`` / ``enqueue()`` can be called from any thread.
    The async flush loop runs on the event loop captured at ``start()``.

    Satisfies the ``TelemetrySink`` protocol so it can be plugged directly
    into ``set_telemetry_sink()``.
    """
    ...
    def emit(self, event: dict[str, Any]) -> None:
        """TelemetrySink protocol: thread-safe, non-blocking enqueue."""
        self.enqueue(event)
