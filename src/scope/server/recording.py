...
class RecordingTrack(MediaStreamTrack):
    ...
    def __init__(self, source_track: MediaStreamTrack, is_paused: callable):
        super().__init__()
...
class RecordingManager:
    ...
    def _create_recording_track(self) -> RecordingTrack:
        ...
        return RecordingTrack(source_track, self.is_paused)
        ...
        try:
            ...
            if os.path.exists(recording_file):
                with self.recording_lock:
                    ...
                    logger.info(f"Added recording segment to list: {recording_file}")
        except Exception as e:
            logger.error(
    ...
    async def _concatenate_segments_for_download(self):
        """Concatenate all recording segments into a single file for download."""
        with self.recording_lock:
        ...
        if len(existing_segments) == 1:
            return self._copy_single_segment(existing_segments[0])

        return await self._concatenate_multiple_segments(existing_segments)
