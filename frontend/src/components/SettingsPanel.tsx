...
import { ArrowUp, ArrowDown, Hammer, Info, Minus, Plus, RotateCcw } from "lucide-react";
            ...
            <SelectContent>
              {pipelines &&
                Object.entries(pipelines)
                  .filter(([, info]) => info.usage?.includes("pipeline") ?? true)
                  .map(([id]) => (
                    ...
                    <Button
                      ...
                      disabled={
                        ...
                        !Object.values(pipelines || {}).some(
                          (p) => {
                            const isPreprocessor = p.usage?.includes("preprocessor") ?? false;
                            if (!isPreprocessor) return false;
                            // Filter by input mode: only show preprocessors that support the current input mode
                            if (inputMode) {
                              return p.supportedModes?.includes(inputMode) ?? false;
                            }
                            return true;
                          }
                        ) ||
