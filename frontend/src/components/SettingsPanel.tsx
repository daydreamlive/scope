...
export function SettingsPanel({
  ...
  const resolutionWarning =
    scaleFactor &&
    (effectiveResolution.height % scaleFactor !== 0 ||
      effectiveResolution.width % scaleFactor !== 0)
      ? `Resolution will be adjusted to ${adjustResolutionForPipeline(pipelineId, effectiveResolution).resolution.width}×${adjustResolutionForPipeline(pipelineId, effectiveResolution).resolution.height} when starting the stream (must be divisible by ${scaleFactor})`
      : null;
