...
export const PIPELINES: Record<string, PipelineInfo> = {
  streamdiffusionv2: {
    ...
    about:
      "A streaming pipeline and autoregressive video diffusion model from the creators of the original StreamDiffusion project. The model is trained using Self-Forcing on Wan2.1 1.3b with modifications to support streaming. Includes VACE (Video-Aware Condition Encoding) for reference image conditioning in text mode.",
    modified: true,
    estimatedVram: 20,
    requiresModels: true,
  ...
  longlive: {
    ...
    about:
      "A streaming pipeline and autoregressive video diffusion model from Nvidia, MIT, HKUST, HKU and THU. The model is trained using Self-Forcing on Wan2.1 1.3b with modifications to support smoother prompt switching and improved quality over longer time periods while maintaining fast generation. Includes VACE (Video-Aware Condition Encoding) for reference image conditioning.",
    modified: true,
    estimatedVram: 20,
    requiresModels: true,
    defaultTemporalInterpolationMethod: "slerp",
