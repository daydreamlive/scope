...
class CudaGraphRecaptureLoRAManager:
    ...
    @staticmethod
    def get_wrapped_model(model: nn.Module) -> nn.Module | None:
        """
    ...
    def _inject_lora_layers(
        ...
        if state["peft_model"] is not None:
            ...
            def wrapped_forward(*args, **kwargs):
                logger.debug(
                    "_inject_lora_layers.wrapped_forward: Intercepted PEFT model forward"
                )
            ...
            logger.info(
                "_inject_lora_layers: Wrapped PEFT model's forward() for lazy CUDA Graph capture"
            )
    ...
    def _clone_tensor_structure(obj):
        ...
        if isinstance(obj, torch.Tensor):
            ...
            return [
                CudaGraphRecaptureLoRAManager._clone_tensor_structure(item)
                for item in obj
            ]
            ...
            return {
                k: CudaGraphRecaptureLoRAManager._clone_tensor_structure(v)
    ...
    def _copy_tensor_data(dest, src):
        ...
        if isinstance(dest, torch.Tensor) and isinstance(src, torch.Tensor):
            ...
            for d, s in zip(dest, src, strict=False):
    ...
    def _capture_cuda_graph(
        model: nn.Module, original_call, args: tuple, kwargs: dict
    ) -> None:
        """
        ...
        logger.info(
            f"_capture_cuda_graph: Capturing graph with {len(args)} args and {len(kwargs)} kwargs"
        )
        ...
        logger.info("_capture_cuda_graph: Graph captured successfully")

    @staticmethod
    def _replay_cuda_graph(model: nn.Module, args: tuple, kwargs: dict):
        """
    ...
    @staticmethod
    def _lazy_graph_forward(
        peft_model: nn.Module, original_call, state: dict[str, Any], *args, **kwargs
    ):
        """
        ...
        """
        logger.debug(
            f"_lazy_graph_forward: Called with args={len(args)}, kwargs={list(kwargs.keys())}"
        )
        logger.debug(
            f"_lazy_graph_forward: State - captured={state['captured']}, capture_on_next_call={state.get('capture_on_next_call', False)}"
        )
        ...
        if state.get("capture_on_next_call", False) and not state["captured"]:
            ...
            logger.info(
                "_lazy_graph_forward: Attempting CUDA Graph capture on first forward pass"
            )
            logger.info(
                f"_lazy_graph_forward: Args: {len(args)}, Kwargs keys: {list(kwargs.keys())}"
            )
            ...
            logger.info("_lazy_graph_forward: CUDA Graph captured successfully")

        # If graph is captured, use it
        if state["captured"]:
            logger.debug("_lazy_graph_forward: Using captured CUDA Graph for inference")
            return CudaGraphRecaptureLoRAManager._replay_cuda_graph(
                peft_model, args, kwargs
            )

        # Should not reach here
        raise RuntimeError(
            "_lazy_graph_forward: Unexpected state - graph not captured and capture not scheduled"
        )
    ...
    def load_adapter(
        ...
        if capture_shape is not None:
            if not torch.cuda.is_available():
                logger.warning(
                    "load_adapter: CUDA not available, skipping graph capture"
                )
            else:
    ...
    def load_adapters_from_list(
        ...
        if capture_shape is not None and loaded_adapters:
            ...
            if state["peft_model"] is not None and torch.cuda.is_available():
                ...
                logger.warning(
                    f"{logger_prefix}CUDA not available, skipping graph capture"
                )
    ...
    def update_adapter_scales(
        ...
        if updates_applied > 0 and state["captured"]:
            logger.info(
                f"{logger_prefix}Re-capturing CUDA Graph after scale updates..."
            )
            ...
            if original_call is None:
                logger.warning(
                    f"{logger_prefix}No original_call stored, using current peft_model.__call__"
                )
