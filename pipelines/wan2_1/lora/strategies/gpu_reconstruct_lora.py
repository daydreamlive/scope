...
class GpuReconstructLoRAManager:
    ...
    @staticmethod
    def _backup_original_weights(
        model: torch.nn.Module, keys_to_backup: list[str]
    ) -> None:
        """
        ...
        logger.debug(
            f"_backup_original_weights: Backed up {len(state['original_weights'])} weights on GPU"
        )
    ...
    def _apply_all_loras(model: torch.nn.Module, initial_load: bool = False) -> None:
        ...
        for key, original in state["original_weights"].items():
            ...
            for _path, lora_info in state["loras"].items():
    ...
    def load_adapters_from_list(
        ...
        for lora_config in lora_configs:
            ...
            try:
                ...
                logger.info(
                    f"{logger_prefix}Loaded LoRA '{Path(lora_path).name}' (scale={scale}) in {elapsed:.3f}s"
                )
        ...
        for path, new_scale in scale_map.items():
            if path in state["loras"]:
                ...
                if abs(old_scale - new_scale) > 1e-6:
                    ...
                    scales_changed = True
                    logger.info(
                        f"{logger_prefix}Updated LoRA '{Path(path).name}' scale from {old_scale} to {new_scale}"
                    )
