...
\begin{itemize}
    ...
    \item Empirical validation that structural control (depth, scribble, optical flow, layout), masked generation (inpainting, outpainting), and temporal extension all function at 20+ fps on consumer hardware in streaming contexts.
    \item A pipeline mixin design validated across five distinct Wan-based autoregressive pipelines without per-model modifications.
\end{itemize}
...
\begin{table}[h]
\centering
\caption{Compatible base pipelines.}
\label{tab:pipelines}
\begin{tabular}{ll}
\toprule
...
To validate cross-pipeline compatibility, we benchmark on two distinct base pipelines: LongLive (4 denoising steps, 12 frames per chunk) and StreamDiffusion V2 (2 denoising steps, 4 frames per chunk). The same VACE adaptation code is used for both without modification.
...
\begin{table}[h]
\centering
\caption{StreamDiffusion V2 1.3B ablation (4 frames per chunk, 2 denoising steps).}
\label{tab:perf-sdv2}
\begin{tabular}{lcccc}
\toprule
\textbf{Configuration} & \textbf{Avg Latency} & \textbf{Avg FPS} & \textbf{Peak FPS} & \textbf{Peak VRAM} \\
\midrule
Baseline (no VACE) & 233\,ms & 17.2 & 17.7 & 12.6\,GB \\
+ Depth Control & 597\,ms & 6.7 & 6.8 & 15.2\,GB \\
+ Inpainting & 557\,ms & 7.2 & 7.3 & 15.3\,GB \\
\bottomrule
\end{tabular}
\end{table}

VACE adds approximately 1.4--2.7\,GB of VRAM overhead across pipelines (the VACE Context Block weights). Per-chunk latency overhead varies by base pipeline and mode. With LongLive (4 denoising steps), VACE context encoding adds ${\sim}$30\% latency for depth and inpainting. Extension mode caches reference hints after the first chunk, resulting in negligible overhead (${\sim}$1\%) for subsequent chunks.

With StreamDiffusion V2 (2 denoising steps), the same VACE context encoding represents a larger fraction of total compute, resulting in ${\sim}$170\% latency overhead for depth control. This is expected: the fixed cost of VACE encoding is constant regardless of the number of denoising steps, so pipelines with fewer steps see proportionally higher overhead. Optimizations such as compiling the VACE encoder or reducing its precision could mitigate this.
...
By moving reference frames from the diffusion latent space into a parallel conditioning pathway, this adaptation preserves the fixed chunk sizes and KV caching that autoregressive models require while reusing existing VACE weights directly. Structural control, masked generation, and temporal extension function in real-time at 20+ fps on consumer hardware. The approach has been validated across multiple Wan-based autoregressive pipelines without per-model modifications.
